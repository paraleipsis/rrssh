...
class ReverseSSHServerFactory(asyncssh.SSHServer):
    ...
    paths = {request_type: dict() for request_type in ALL_TYPES}
    ...
    def connection_requested(
        ...
        return ReverseSSHServerSession(
            paths=ReverseSSHServerFactory.paths,
...
class ReverseSSHServer:
    ...
    @staticmethod
    def path(request_type: str, resource: str, callback: callable) -> None:
        ...
        if resource not in ReverseSSHServerFactory.paths[request_type]:
            ReverseSSHServerFactory.paths[request_type][resource] = callback
