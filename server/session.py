...
class ReverseSSHServerSession(asyncssh.SSHTCPSession):
    ...
    async def __process_request(self, request: MutableMapping) -> None:
        if request['request_type'] not in self._paths:
            logger['debug'].debug(
                f"No path found for {request['request_type']}"
            )
            self._send_response(request['id'], 404)
            return None

        if request['router'] not in self._paths[request['request_type']]:
            logger['debug'].debug(
                f"No path found for {request['request_type']} on {request['router']}"
            )
            self._send_response(request['id'], 404)
            return None

        path = self._paths[request['request_type']][request['router']]
        ...
        try:
            response = await path(params=params, data=data, **kwargs)
    ...
    async def __process_stream(self, request: MutableMapping) -> None:
        ...
        try:
            async for response in path(params=params, **kwargs):
